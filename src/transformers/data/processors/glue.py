...
class Sentiment3Processor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir_or_url_or_text):
        ...
        if os.path.isfile(devfile):
            ...
            if any(len(d) != 2 for d in devtsv):
                logger.info('not just 2 fields: %s %s' % (devfile, devtsv[0]))
            ...
            logger.info('%s %s' % (devfile, devtsv[0]))
...
glue_processors = {
    ...
    "wnli": WnliProcessor,
    "imdb": Sst2Processor,
    "sentiment3": Sentiment3Processor,
}
